...
func _ready():
	...
	tween.tween_property(self, "position:y", 0.2, 2).as_relative()
	tween.tween_property(self, "position:y", -0.2, 2).as_relative()
...
func _process(delta):
	...
	player_pos = Vector3(owner.back.global_position.x ,owner.back.global_position.y - y_axis, owner.back.global_position.z)
